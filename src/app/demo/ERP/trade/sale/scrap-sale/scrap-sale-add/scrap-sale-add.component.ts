...
import { ErpServiceService } from 'src/app/demo/ERP/erp-service.service';
import { SaleSrviceService } from '../../sale-srvice.service';
import { erp_all_api } from 'src/app/demo/ERP/erpAllApi';
@Component({
  selector: 'app-scrap-sale-add',
  templateUrl: './scrap-sale-add.component.html',
  styleUrls: ['./scrap-sale-add.component.scss']
})
export class ScrapSaleAddComponent implements OnInit {
  ...
  get_proddata = () => {
    ...
    this.ErpService.get_Reqs(erp_all_api.urls.get_scrap_data).pipe(finalize(() => { this.loader = false; })).subscribe(
      (res: any) => {
  ...
  add_purchase_details = () => {
    ...
    this.ErpService.post_Reqs(erp_all_api.urls.scrap_sell_entry, reqBody).pipe(finalize(() => { this.loader = false; })).subscribe(
      (res: any) => {
  ...
  get_next_invoice = () => {
    ...
    // let headers = new HttpHeaders();
    // headers = headers.set('auth-token', auth_token);
    this.ErpService.get_Reqs(erp_all_api.urls.get_next_scrap_invoice_no).pipe(finalize(() => { this.loader = false; })).subscribe(
